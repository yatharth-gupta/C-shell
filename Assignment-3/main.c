...
int main()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i <= command_semicolon && c_flag == 0; i++)
        {
            // puts("hi1");
            signal(SIGINT, C_handle);
            int comm_length = strlen(token_semi[i]);
            ...
            for (int j = 0; j < comm_length; j++)
            {
                if (token_semi[i][j] == '>')
                {
                    ...
                    if (check == 0)
                    {
                        strncpy(command, token_semi[i], j);
                        command[j] == '\0';
                        if (token_semi[i][j + 1] == '>')
                        {
                            flag_append = 1;
                            strcpy(file1, &token_semi[i][j + 2]);
                        }
                        else
                        {
                            strcpy(file1, &token_semi[i][j + 1]);
                        }
                    }
                    else
                    {
                        strncpy(file1, &token_semi[i][check], j - check);
                        file1[j - check] == '\0';
                        if (token_semi[i][j + 1] == '>')
                        {
                            flag_append = 1;
                            strcpy(file2, &token_semi[i][j + 2]);
                        }
                        else
                        {
                            strcpy(file2, &token_semi[i][j + 1]);
                        }
                    }
                    // command_no = j;
                }
                else if (token_semi[i][j] == '<')
                {
                    flag_redirection = 1;
                    strncpy(command, token_semi[i], j);
                    command[j] == '\0';
                    check = j + 1;
                    strcpy(file1, &token_semi[i][check]);
                    // file1[j - check] == '\0';
                    // if (token_semi[i][j + 1] == '>')
                    // {
                    //     flag_append = 1;
            ...
            else
            {
                // puts("hi");
                int count = 0;
                for (int s = 0; s < strlen(token_semi[i]); s++)
                {
                    if (token_semi[i][s] == '&')
                    {
                        count++;
                ...
                strcpy(input_copy, token_semi[i]);
                char *token_and[1000];
                for (int p = 0; p < 1000; p++)
                {
